...
export const useTransferStore = create<TransferStore>()(
  immer((set, get) => {
    const { hasExtension } = useDashboardStore.getState();

    const initialWalletTypes: WalletType[] = [
    ...
    return {
      step: 1,
      loading: false,
      walletTypes: hasExtension("futures")
        ? initialWalletTypes
        : initialWalletTypes.filter((type) => type.value !== "FUTURES"),
